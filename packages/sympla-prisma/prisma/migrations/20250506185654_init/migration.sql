...
CREATE TABLE `Atividade` (
    ...
    `tipoAtividadeMobile` ENUM('ivItIu', 'mpdj', 'mpbb') NOT NULL,
    `ordemServico` VARCHAR(191) NULL,
    `status` ENUM('PENDENTE', 'EM_EXECUCAO', 'CONCLUIDA', 'CANCELADA') NOT NULL DEFAULT 'PENDENTE',
    `prazoLimite` DATETIME(3) NULL,
    `dataProgramacao` DATETIME(3) NULL,
...
-- CreateTable
CREATE TABLE `ChecklisrPerguntaRelation` (
    `id` VARCHAR(191) NOT NULL,
    `perguntaId` VARCHAR(191) NOT NULL,
    `ordem` INTEGER NOT NULL DEFAULT 0,
    `modeloId` VARCHAR(191) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `NotasSAP` ADD CONSTRAINT `NotasSAP_kpiId_fkey` FOREIGN KEY (`kpiId`) REFERENCES `Kpi`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `HistoricoAlteracaoNota` ADD CONSTRAINT `HistoricoAlteracaoNota_notaId_fkey` FOREIGN KEY (`notaId`) REFERENCES `NotasSAP`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `ChecklisrPerguntaRelation` ADD CONSTRAINT `ChecklisrPerguntaRelation_modeloId_fkey` FOREIGN KEY (`modeloId`) REFERENCES `ChecklistModelo`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
