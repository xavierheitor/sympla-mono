...
import {
  CentroTrabalhoFormData,
  CentroTrabalhoWithRelations,
} from '@/lib/actions/centroTrabalho/schema';
import {
  createCentroTrabalho,
  deleteCentroTrabalho,
  getAllCentroTrabalhosWithIncludes,
  updateCentroTrabalho,
} from '@/lib/actions/centroTrabalho/actionsCentroTrabalho';
...
export default function CentroTrabalhoPage() {
  ...
  const {
    data: centrosTrabalho,
    isLoading,
    error,
  } = useServerData('centroTrabalho', unwrapFetcher(getAllCentroTrabalhosWithIncludes));
  ...
  const columns = useTableColumnsWithActions<CentroTrabalhoWithRelations>(
    ...
    ],
    {
      ...
      onDelete: (item) =>
        controller.exec(
          () => deleteCentroTrabalho(item.id),
          'Centro de trabalho excluído com sucesso!',
        ),
    },
  );
