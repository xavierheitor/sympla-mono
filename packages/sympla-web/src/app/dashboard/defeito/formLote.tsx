...
export default function DefeitoLoteForm({ grupoOptions, subgrupoOptions, onSuccess }: Props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'equipe',
      render: (text: string, record: LinhaDefeito, index: number) => (
        <Input value={text} onChange={(e) => handleInputChange(e.target.value, index, 'equipe')} />
      ),
    },
  ];
      ...
      >
        ...
        >
          <span style={{ color: isFocused ? '#1677ff' : '#999' }}>
            {isFocused
              ? 'Pronto! Cole agora seus dados (CTRL + V)'
              : 'Clique aqui e pressione CTRL + V para colar dados do Excel'}
          </span>
        </div>
