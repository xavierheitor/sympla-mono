...
import {
  ...
  updateDefeito,
} from '@/lib/actions/defeito/actionsDefeito';
  ...
  const columns = useTableColumnsWithActions<DefeitoWithRelations>(
    [
      ...
      },
      {
        ...
        render: (_, record) =>
          record.grupo.codigo ? `${record.grupo.codigo} - ${record.grupo.nome}` : record.grupo.nome,
      },
      {
        title: 'Subgrupo',
    ...
    ],
    {
      ...
      onDelete: (item) =>
        controller
          .exec(() => deleteDefeito(item.id), 'Defeito excluído com sucesso!')
