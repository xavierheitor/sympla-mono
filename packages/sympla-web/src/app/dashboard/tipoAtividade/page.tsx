...
import {
  ...
  getAllTipoAtividadeEnums,
  setTipoAtividadeKpi,
} from '@/lib/actions/atividade/actionsTipoAtividade';
...
export default function TipoAtividadePage() {
  ...
  const { data: tipoMobileOptions } = useServerData(
    'tipoAtividadeMobileEnum',
    getAllTipoAtividadeEnums,
  );
  ...
  const columns = useTableColumnsWithActions<TipoAtividadeWithKpi>(
    ...
    ],
    {
      ...
      onDelete: (item) =>
        controller
          .exec(() => deleteTipoAtividade(item.id), 'Tipo de Atividade excluído com sucesso!')
          .finally(() => tipoAtividades.mutate()),
    },
  );
  ...
  const handleSubmit = async (values: TipoAtividadeFormData & { kpiIds: string[] }) => {
    ...
    const action = async () => {
      ...
      await setTipoAtividadeKpi({
        tipoAtividadeId: tipoAtividade.data!.id,
        kpiIds,
      });
    ...
    controller
      .exec(action, 'Tipo de Atividade salvo com sucesso!')
      .finally(() => tipoAtividades.mutate());
  };
