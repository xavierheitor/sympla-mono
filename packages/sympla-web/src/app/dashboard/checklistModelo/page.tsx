...
export default function ChecklistModeloPage() {
  ...
  const { data: tipoAtividades } = useServerData(
    'tipoAtividades',
    unwrapFetcher(getAllTipoAtividades),
  );
  const { data: perguntas } = useServerData(
    'checklistPerguntas',
    unwrapFetcher(getAllChecklistPerguntas),
  );
  ...
  const columns = useTableColumnsWithActions<ChecklistModelo>(
    ...
    ],
    {
      ...
      onDelete: (item) =>
        controller
          .exec(() => deleteChecklistModelo(item.id), 'Modelo excluído com sucesso!')
          .finally(() => {
            modelos.mutate();
          }),
    },
