...
import { createPrismaCreateAction, createPrismaDeleteAction, createPrismaGetAllAction, createPrismaGetAllWithIncludesAction, createPrismaUpdateAction } from "@/lib/server-action/actionFactory";
import { notaPmaFormSchema, NotaPMALoteInput, notaPmaLoteSchema, NotaPMAWithRelations } from "./schema";
...
//*GET ALL
export const getAllNotasPMA = createPrismaGetAllAction(
    prisma.notaPMA,
    "NOTA_PMA",
    ["numeroNota", "localInstalacao", "dataInicioPlan", "dataFiPlan", "dataInicioReal", "dataFiReal", "centroTrabalhoId", "equipamentoId", "kpiId", "status"]
)

//*GET ALL WITH INCLUDES
export const getAllNotasPMAWithIncludes = createPrismaGetAllWithIncludesAction<NotaPMAWithRelations>(
    async (params) => {
        const {
            where = {},
            orderBy = "numeroNota",
        ...
        const finalWhere = {
            ...
            ...(filtroLocalInstalacao && { localInstalacao: { contains: filtroLocalInstalacao } }),
            ...(filtroDataInicioPlan && { dataInicioPlan: { gte: filtroDataInicioPlan } }),
            ...(filtroDataFiPlan && { dataFiPlan: { lte: filtroDataFiPlan } }),
            ...(filtroDataInicioReal && { dataInicioReal: { gte: filtroDataInicioReal } }),
            ...(filtroDataFiReal && { dataFiReal: { lte: filtroDataFiReal } }),
            ...(filtroCentroTrabalhoId && { centroTrabalhoId: { in: filtroCentroTrabalhoId } }),
            ...(filtroEquipamentoId && { equipamentoId: { in: filtroEquipamentoId } }),
