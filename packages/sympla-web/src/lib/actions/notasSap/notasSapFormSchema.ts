import { z } from 'zod';
import { NotasSAP, PrioridadeDefeito, StatusNota, TipoNota } from '@sympla/prisma';
...
type BaseFields = Required<
  Pick<NotasSAP, 'descricao' | 'notificador' | 'localInstalacao' | 'tipoNota' | 'dataNota' | 'centroTrabalhoId' | 'regionalId' | 'status'>
> &
  Partial<
    Pick<
      NotasSAP,
      'numeroNota' | 'dataInicioPlan' | 'dataFiPlan' | 'dataInicioReal' | 'dataFiReal' | 'equipamentoId' | 'kpiId' | 'prioridade' | 'ordemServicoExecucao'>
  >;
